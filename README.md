...
1. **ClientHello** — клієнт передає підтримувані версії TLS, шифронабори та SNI.
2. **ServerHello** — сервер обирає версію TLS та алгоритм шифрування.
3. **Certificate & Key Exchange** — сервер передає сертифікат і відбувається формування спільних ключів.
4. **Finished** — після завершення узгодження використовується симетричне шифрування, наприклад AES-GCM або ChaCha20.
